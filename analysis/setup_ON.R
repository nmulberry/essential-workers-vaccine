...
n <- length(age_demo)
pop_total <- age_demo[n] #pop ON now
...
#Get rolling sum for case counts instead of incid
case_dat <- readRDS('../data/cases_ON.RDS')
test = case_dat %>% group_by(new_age_group) %>% 
  mutate(prev = RcppRoll::roll_sum(value,5, fill=NA, align="right")) 

I0 = filter(test , date == startDate)$prev
# fix to account for essential workers 
# assign evenly based on demo data?
I_0 <- rep(0,num_groups)

# case data doesn't have 0-9 :/
I0<- c(0.5*I0[1],0.5*I0[1], I0[2:8])

I_0[1:2] <- I0[1:2]
I_0[3:8] <- 0.6*I0[3:8]
I_0[9] <- I0[9]
I_0[10:15] <- 0.4*I0[3:8]

#u_var <- c(0.38, 0.4, 0.79, 0.8, 0.8, 0.82, 0.88,1.0, 1.0,
#  0.79, 0.8, 0.8, 0.82, 1.0,1.0) # rel. susceptibility (this works better with validation than one in bubar)
u_var <- c(0.38, 0.4, 0.7, 0.7, 0.7,0.7,1.0,1.0, 1.0,
0.7, 0.7, 0.7, 0.7,1.0,1.0) # FITTED FOR ONTARIO (but does not affect results much)
...
labels <- c('A: Oldest to Youngest', 
       #     'B: 80+, 20-79',
            'B: 80+, EW, 70-79,...',  #was C 
       #     'D: 80+, EW, 20-79',
            'C: 80+, EW, 70-79, 20-69', # was E
            'D 80+, EW&70-79, 60-69, ...') # was F
...
run_over_scen_rate = function(R, ve,ratefactor, scen,alpha=0.0){
   T1 <- 60
   T2 <- 210
   vp=0.9
   # Initial stage (vax all 80+)
   ...
   df0 <- run_sim_basic(C, I_0=I_0, percent_vax =1.0, strategy=list(9), num_perday=n,
                        ...
                        u = u_var, num_days=T1, with_essential=TRUE, H=H) 
   # Final stage
   n <- ratefactor*sum(age_demo[-9])/T2
   C <- construct_C_from_prem(home=mu_home, work=mu_work, school=mu_school, other=mu_other, u=u_var,
                              target_R0=R, in_school=TRUE, alpha_factor=alpha)
   df <- run_sim_restart(C, df_0=tail(df0, n=1), percent_vax =1.0, strategy= strategies[[scen]], num_perday=n,
                         v_e = rep(ve, num_groups), v_p=rep(vp, num_groups),
